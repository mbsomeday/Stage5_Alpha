import torch
# from CAM.vgg import VGG
import torch.nn.functional as F

# from CAM_Beta.VGG import VGG

from CAM_Org.models.vgg import VGG
...
class FG_CAM:
    ...
    def find_layer(self, target_num):
        if isinstance(self.model, VGG):
            return self.model.features[target_num]
    ...
    def get_weight_by_score_cam(self, input, target_class, layer):
        ...
        with torch.no_grad():
            ...
            maxs = saliency_map.view(saliency_map.size(0), saliency_map.size(1), -1).max(dim=-1)[0]
            mins = saliency_map.view(saliency_map.size(0), saliency_map.size(1), -1).min(dim=-1)[0]
    ...
    def forward(self, input, denoising, target_layer, target_class):
        ...
        I = self.get_explanation_component(input, target_class, target_layer)
